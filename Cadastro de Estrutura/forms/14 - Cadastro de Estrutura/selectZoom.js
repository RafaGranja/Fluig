...
function setSelectedZoomItem(selectedItem) {        
	...
	if(!(selectedItem.inputId==undefined)){
		...
		if(selectedItem.inputId.indexOf("OS_INFO")!="-1"){
			...
			//reloadZoomFilterValues("CODIGOTAREFA", "IDPRJ,"+selectedItem["IDPRJ"]);
		...
		if(selectedItem.inputId.indexOf("INFOCOPIA")!="-1"){
			...
			$("#DESCRICAO_OS_COPIA").val(selectedItem['DESCRICAO'])
		...
		if(selectedItem.inputId.indexOf("CODIGOTAREFA")!="-1"){
			...
			var tarefa = selectedItem['NOME'].toString().replace(/[',"]/g," ")
	...
	if(selectedItem.type == "tarefa") {
		...
		$("#F_CODIGOTAREFA").val(selectedItem.CODTRF+" - "+tarefa)
		//$("#F_PRODUTO_RM").prop("readonly",true)
	...
	if(selectedItem.type == "tarefaPai") {
		...
		console.log("entrei tarefaPai")

		if(!verificaCodTarefa(selectedItem.CODTRF)){

			// EXIBE ALERTA
			Swal.fire({
				icon: 'error',
				title: 'Este código já foi selecionado na estrutura desta OS!',
			...
			$("#F_CODIGOTRFPAI").val(selectedItem.CODTRF+" - "+tarefa)
			//$("#F_PRODUTO_RM").prop("readonly",true)
			console.log("TAREFA: "+selectedItem.TAREFA+", IDTRF: "+selectedItem.IDTRF+", CODTRF: "+selectedItem.CODTRF+", NOMETRF: "+selectedItem.NOME)
			$("#F_CODTRFPAI").val(selectedItem.CODTRF)
			$("#F_IDTRFPAI").val(selectedItem.IDTRF)
			$("#F_NOMETRFPAI").val(tarefa)
	...
	if(selectedItem.type == "atividade") {
		...
		if(verificaAtividade(selectedItem.CODATIVIDADE)){
			...
			var atividade = selectedItem.DSCATIVIDADE.toString().replace(/[',"]/g," ")
			
			$("#VIEWATIVIDADE___"+seq).val(""+selectedItem.CODATIVIDADE+" - "+atividade)
			$("#VIEWCODATIVIDADE___"+seq).val(selectedItem.CODATIVIDADE)
			$("#VIEWDESCATIVIDADE___"+seq).val(atividade)
			...
			if(!(selectedItem.CODHABILIDADE==null || selectedItem.CODHABILIDADE=="" || selectedItem.CODHABILIDADE=="null")){
				...
				var habilidade = selectedItem.DESCRICAOHABIL.toString().replace(/[',"]/g," ")
				
				$("#VIEWHABILIDADEREQUERIDA___"+seq).val(""+selectedItem.CODHABILIDADE+" - "+habilidade)
	...
	if(selectedItem.type == "posto") {
		...
		var posto = selectedItem.DSCPOSTO.toString().replace(/[',"]/g," ")
		
		$("#VIEWPOSTO___"+seq).val(""+selectedItem.CODPOSTO+" - "+posto)
		$("#VIEWCODPOSTO___"+seq).val(selectedItem.CODPOSTO)
		$("#VIEWDESCPOSTO___"+seq).val(posto)
...
function removedZoomItem(removedItem)
{
	...
	// SE A OS DA CÓPIA É REMOVIDA
	if(removedItem.inputId.indexOf("INFOCOPIA")!="-1")
	{
