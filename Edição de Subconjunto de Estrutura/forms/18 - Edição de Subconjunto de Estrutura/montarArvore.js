...
function retornaArrayIndicesSemOrd(){
	...
	if(indicePai=="1"){
		...
		//var c3 = DatasetFactory.createConstraint("ORDENACAO",'1','1',ConstraintType.MUST)

		var constraints = new Array(c1)
		
		//dataset = DatasetFactory.getDataset("dsBuscaEstrutura",null,constraints,null)
		var dataset = DatasetFactory.getDataset("dsBuscaEstruturaSemOrd",null,constraints,null)
...
function idTemIrmaoEsq(indice,indices,op){
	...
	while(posicao>=0 && flag){
		...
		if(!(indices.indexOf(irmaoEsq)==-1)){
			...
			console.log("tem irmao esquerda SIM")
	...
	if(indices.indexOf(irmaoEsq)==-1){
		...
		console.log("irmao não tem irmão a esquerda")
	...
	if(op==0){
		...
		console.log("irmão a esquerda: "+temIrmaoEsquerda)
	...
	if(op==1){
		...
		console.log("irmao tem irmão a esquerda, op 1")
...
function sobeIndice(indice,indices){
	...
	if(idTemIrmaoEsq(indice,indices,0)){
		...
		indices = retornaArrayIndicesSemOrd()
...
function avancaIndice(indice,indices){
	...
	if(idTemIrmaoEsq(indice,indices,0) && !(nivelRef=="" || nivelRef==null || nivelRef==undefined) ){
		...
		// SE IRMAO A ESQUERDA NÃO É UM NÃO MANUFATURADO OU INDUSTRIALIZADO
...
function incluirFilho(indice){
	...
	if(itemNaoManufaturado(indice) || itemIndustrializado(indice)){
		...
		Swal.fire({
			icon: 'error',
			title: 'Este é um item Não Manufaturado ou Industrializado e não pode ter filhos',
			text: 'Verifique e tente novamente.'
	  })
...
function alterarItem(indice,arrayIdCriacao){
	...
	if(!(row=="" || row==null || row==undefined || row=="null")){
		...
		if(rep["NIVEL"]=="" || rep["NIVEL"]==null || rep["NIVEL"]==undefined || rep["NIVEL"]=="null"){
			...
			$(".CODTRFPAI").show()
			$("#F_CODIGOTRFPAI").prop("readonly",false)
...
function confirmaAlteracao(){
	...
	setTimeout(function(){
		...
		if(numDesenho=="" || descricao=="" || posicao=="" || desQtde=="" || execucoes==""){
			...
			if(tam>119){
				...
				if(nivel=="" || nivel==undefined || nivel==null){
					...
					var a12 = DatasetFactory.createConstraint("RECCREATEDBY",reccreatedby,reccreatedby,ConstraintType.MUST);
					var a13 = DatasetFactory.createConstraint("RECMODIFIEDBY",recmodifiedby,recmodifiedby,ConstraintType.MUST);

					var constraints = new Array(a1,a2,a3,a4,a5,a6,a7,a8,a9,a10,a11,a12,a13)
					...
					var a6 = DatasetFactory.createConstraint("DESQTDE",desQtde,desQtde,ConstraintType.MUST)
					var a7 = DatasetFactory.createConstraint("TOTALQTDE",totalQtde,totalQtde,ConstraintType.MUST)
					var a8 = DatasetFactory.createConstraint("EXECUCOES",execucoes,execucoes,ConstraintType.MUST)
					var a12 = DatasetFactory.createConstraint("RECCREATEDBY",reccreatedby,reccreatedby,ConstraintType.MUST);
					var a13 = DatasetFactory.createConstraint("RECMODIFIEDBY",recmodifiedby,recmodifiedby,ConstraintType.MUST);
					
					var constraints = new Array(a1,a2,a3,a4,a5,a6,a7,a8,a12,a13)
					var dataset = DatasetFactory.getDataset("dsUpdateCabecalhoEstruturaOS",null,constraints,null)
...
function trocaIndice(indice,idCriacao){
	...
	if(nivel=="" || nivel==undefined || nivel==null){
		...
		reloadZoomFilterValues("F_INDICE_REF","OS,"+numOS+"NIVEL,"+nivel+",IDCRIACAO,"+idCriacaoPai+",NOTINDICE,"+indice+",INDICEPAI,"+indicePai)
		...
		// FAZ O RELOAD ZOOM NA CONSULTA DOS INDICES
		reloadZoomFilterValues("F_INDICE_REF","OS,"+numOS+",IDCRIACAO,"+idCriacaoPai+",NOTINDICE,"+indice+",INDICEPAI,"+indicePai)
