...
function createDataset(fields, constraints, sortFields) {
   ...
   /* var myQuery =   "SELECT "+
					"	O.CODCCUSTO OS, O.CODORDEM OP, KA.CODATIVIDADE, KA.DSCATIVIDADE, N.DESCRICAO ITEM, N.MATERIAL , C.POSICAO, C.BITOLA , A.IDATVORDEM, C.CODESTRUTURA, Z.NUMPLANOCORTE, N.NUMDESENHO, I.QTDEPLANEJADA "+
					"FROM "+
					"	KORDEM O "+
					"	INNER JOIN KITEMORDEM I ON O.CODCOLIGADA = I.CODCOLIGADA AND O.CODFILIAL = I.CODFILIAL AND O.CODORDEM = I.CODORDEM "+
					"	INNER JOIN KESTRUTURACOMPL C ON C.CODCOLIGADA = I.CODCOLIGADA AND C.CODFILIAL = I.CODFILIAL AND C.CODESTRUTURA = I.CODESTRUTURA "+
					"	INNER JOIN KATVORDEM A ON A.CODCOLIGADA = I.CODCOLIGADA AND A.CODFILIAL = I.CODFILIAL AND A.CODORDEM = I.CODORDEM AND A.CODESTRUTURA = I.CODESTRUTURA "+
					"	INNER JOIN KATIVIDADE KA ON A.CODCOLIGADA = KA.CODCOLIGADA AND A.CODFILIAL = KA.CODFILIAL AND A.CODATIVIDADE = KA.CODATIVIDADE "+
					//"	INNER JOIN FLUIG_TESTE.DBO.ML001004 M ON M.NUM_OS = O.CODCCUSTO COLLATE Latin1_General_CI_AS "+
					//"	INNER JOIN FLUIG_TESTE.DBO.Z_CRM_ML001005 N ON M.documentid = N.documentid AND M.version = N.version AND N.CODIGOPRD = I.CODESTRUTURA COLLATE Latin1_General_CI_AS "+
					"	INNER JOIN FLUIG_TESTE.DBO.Z_CRM_ML001005 N ON N.OS = O.CODCCUSTO COLLATE Latin1_General_CI_AS AND N.CODIGOPRD = I.CODESTRUTURA COLLATE Latin1_General_CI_AS "+
					"	LEFT JOIN ZMDPLANODECORTE Z ON O.CODCOLIGADA = Z.CODCOLIGADA AND O.CODFILIAL = Z.CODFILIAL AND I.CODESTRUTURA = Z.CODESTRUTURA AND A.IDATVORDEM = Z.IDATVORDEM "+
					"WHERE	"+
					"	O.RESPONSAVEL IS NOT NULL "+
					"	AND O.CODCOLIGADA=1 AND O.CODFILIAL=7 "+
					"	AND		O.CODCCUSTO IS NOT NULL "+
					//"	AND ( KA.DSCATIVIDADE LIKE '%CORTAR MANUAL%' OR KA.DSCATIVIDADE LIKE '%SERRAR%' OR KA.DSCATIVIDADE LIKE '%OXICORTAR%' OR KA.DSCATIVIDADE LIKE '%CORTAR A PLASMA%')		"+
					" "+clausulaAtv+" "+
					"	AND		O.STATUS NOT IN (4,5,6) "+
					"	AND		C.POSICAO <> '0' "+
					"	AND O.CODCCUSTO='"+os+"' "+
					" "+material+" "+codAtividade+" "+
					"   AND (Z.NUMPLANOCORTE IS NULL "+
					"		OR Z.NUMPLANOCORTE='"+numPlano+"')"+
					"ORDER BY "+
					"	Z.NUMPLANOCORTE DESC "*/
