...
if (constraints != null) {
    ...
    for (var i = 0; i < constraints.length; i++) {
        ...
        if (constraints[i].fieldName == "CELULA") {
        	if(!(constraints[i].initialValue=="" || constraints[i].initialValue==null || constraints[i].initialValue==undefined)){
        		celula = " AND MCL.CELULA='"+constraints[i].initialValue+"' "
        	}
        }
        ...
        if (constraints[i].fieldName == "DATADE") {
        	if(!(constraints[i].initialValue=="" || constraints[i].initialValue==null || constraints[i].initialValue==undefined)){
        		data += " AND CAST(Z.DTHRINICIAL AS DATE)>='"+constraints[i].initialValue+"' "
        	}
        }
		if (constraints[i].fieldName == "DATAATE") {
			if(!(constraints[i].initialValue=="" || constraints[i].initialValue==null || constraints[i].initialValue==undefined)){
				data += " AND CAST(Z.DTHRFINAL AS DATE)<='"+constraints[i].initialValue+"' "
			}
        }
	...
	myQuery =   " SELECT DISTINCT KA.CODCOLIGADA,K.CODCCUSTO,K.STATUS OP_STATUS,KA.STATUS ATV_STATUS,KA.CODESTRUTURA,K.CODORDEM,CASE WHEN MCL.TAG IS NULL THEN 'SEM TAG' ELSE MCL.TAG END AS TAG,K.DSCORDEM," +
				" A.DSCATIVIDADE,A.CODATIVIDADE,KAL.PRIORIDADE,KA.IDATVORDEM,KA.PERCENTUAL,MCL.CELULA,CASE WHEN ZP.CODATIVIDADE IS NULL THEN '0' ELSE ZP.NUMPLANOCORTE END AS PLANODECORTE, " +
				...
				" A.DSCATIVIDADE,A.CODATIVIDADE,KAL.PRIORIDADE,KA.IDATVORDEM,KA.PERCENTUAL,MCL.CELULA,CASE WHEN ZP.NUMPLANOCORTE IS NULL THEN '0' ELSE ZP.NUMPLANOCORTE END AS PLANODECORTE, " +
				" CASE WHEN KAL.PRIORIDADE = (SELECT MAX(PRIORIDADE) FROM KATVORDEMCOMPL WHERE CODORDEM=KA.CODORDEM) THEN 1 ELSE 0 END AS ULTIMAATVOP,KA.TEMPO	"+
				" FROM KORDEM K (NOLOCK) "+
				" INNER JOIN KATVORDEM KA (NOLOCK) ON "+
				" KA.CODCOLIGADA=K.CODCOLIGADA  "+
