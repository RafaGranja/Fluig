...
function beforeTaskSave(colleagueId,nextSequenceId,userList){
	...
	log.info("beforeTaskSave - PROCESSO DE APONTAMENTO DE MATERIA PRIMA");
	
	var ativAtual = getValue("WKNumState");
	var numProcess = getValue("WKNumProces");
	var WKUser = getValue("WKUser");
	var exclusivo = hAPI.getCardValue("EXCLUSIVO1");
	var apont = hAPI.getChildrenIndexes("LISTABAIXASAUX_APONT");
    var papc = hAPI.getChildrenIndexes("LISTABAIXASAUX_PAPC");
	
	log.info("Solicitação: "+numProcess)
	
	log.info("vou percorrer todos os itens da tabela de materia-prima")
	
	log.info("a tabela tem "+apont.length+" itens")
	...
	if(nextSequenceId=="9"){
		...
		log.info("vou baixar materiais")

		// PERCORRE TODOS OS REGISTROS DA TABELA DE APONTAMENTO
		for (var i = 0; i < apont.length; i++) {
			
			log.info("estou no item "+(i+1))
			
			
			// SALVA AS INFORMAÇÕES 
			var numOS = hAPI.getCardValue("CODPRJAUXAPONT___" + apont[i])
			var qtdReal = hAPI.getCardValue("TOTALAPONTAUXAPONT___" + apont[i])
			var idAtv = hAPI.getCardValue("IDATVAUXAPONT___" + apont[i])
			var codColigada = hAPI.getCardValue("CODCOLIGADAAUXAPONT___" + apont[i])
			var codFilial = hAPI.getCardValue("CODFILIALAUXAPONT___" + apont[i])
			var seqAtv = hAPI.getCardValue("SEQAUXAPONT___" + apont[i])
			var codigoPrd = hAPI.getCardValue("CODIGOPRODUTOAUXAPONT___" + apont[i])
			var idprd = hAPI.getCardValue("IDPRDAUXAPONT___" + apont[i])
			var codOrdem = hAPI.getCardValue("OPAUXAPONT___" + apont[i])
			var codEstrutura = hAPI.getCardValue("CODESTRUTURAOPAUXAPONT___" + apont[i])
			
			log.info(" qtdReal: "+qtdReal+
					", idAtv: "+idAtv+", codColigada: "+codColigada+", codFilial: "+codFilial+
					", seqAtv: "+seqAtv+", codigoPrd: "+codigoPrd+", idprd: "+idprd+", idAtv: "+idAtv+", codOrdem: "+
					codOrdem+", codEstrutura: "+codEstrutura+", numOS: "+numOS)
			...
			idmovbx = geraMovBaixa(numProcess, codColigada, codFilial, WKUser, codOrdem, idAtv, seqAtv, numOS)
		...
		log.info("vou baixar planos de corte e aproveitamento")
			
		// PERCORRE TODOS OS REGISTROS DA TABELA DE APONTAMENTO
		for (var i = 0; i < papc.length; i++) {
			
			log.info("estou no item "+(i+1))
			

			var seqAux = hAPI.getCardValue("SEQAUXPAPC___" + papc[i])
			var codColigada = hAPI.getCardValue("CODCOLIGADAAUXPAPC___" + papc[i])
			var codFilial = hAPI.getCardValue("CODFILIALAUXPAPC___" + papc[i])
			var planoCorte = hAPI.getCardValue("PLANOAUXPAPC___" + papc[i])
			
			log.info("planoCorte: "+planoCorte)

			// SE NÃO TEVE APONTAMENTO AINDA
			log.info("vou gerar o movimento de baixa dos componentes do item "+(i+1))
				
			// GERA O MOVIMENTO DE BAIXA DO ESTOQUE
			idmovbx = geraMovBaixaPapc(numProcess, codColigada, codFilial, WKUser, planoCorte)
			
			log.info("vou gerar o movimento de baixa dos componentes do item "+(i+1))

			// GERA O MOVIMENTO DE ENTRADA DE SUCATA
			idmovSucata = geraMovEntradaSucata(numProcess, codColigada, codFilial, WKUser, planoCorte)
...
// GERA O MOVIMENTO DE ENTRADA DE SUCATA
function geraMovEntradaSucata(numProcess, codColigada, codFilial, WKUser, numPlano){
	
	log.info("MOVIMENTO DE ENTRADA DA SUCATA")
	
	log.info("Parâmetros: numProcess: "+numProcess+", codColigada: "+codColigada+", codFilial: "+codFilial+", WKUser: "+WKUser+", numPlano: "+numPlano)

	var c1 = DatasetFactory.createConstraint("TIPO","SUCATA","SUCATA",ConstraintType.MUST)
	...
	log.info("a tabela tem "+rows.length+" itens")
	...
	for (var i = 0; i < rows.length; i++){
		...
		var qtde = dataset.getValue(i,"QTDESUCATA")
		var idprd =dataset.getValue(i,"IDPRD")
		var numOS =dataset.getValue(i,"CODCCUSTO")
		...
		if(!(qtde=="" || qtde==null || qtde==undefined)){
			...
			var XML = 
				...
				"	 <CAMPOLIVRE1>"+dataset.getValue(i,"CODORDEM")+"</CAMPOLIVRE1> "+
				"	 <CAMPOLIVRE2>"+dataset.getValue(i,"IDATVORDEM")+"</CAMPOLIVRE2> "+
				"	 <CAMPOLIVRE3>"+numPlano+"</CAMPOLIVRE3> "+
				"  </TMOV>" +  
			...
			var precoUnit = dataset.getValue(i,"CUSTOMEDIO")
			...
			try{
				...
				salvaIdMovBaixa(1,"1",codloc,result,"SEMLOTE",idprd,qtde,numPlano,codColigada,codFilial)
			...
			if(codPrdAux=="01.020"){
				...
				if(!(qtde=="" || qtde==null || qtde==undefined || qtde=="null")){
					...
					try{
						...
						// SALVA O IDMOVBAIXA GERADO PARA TODOS OS COMPONENTES
						salvaIdMovBaixa(indexesComp,seqAtv,codLocComp,result,idLote,idprd,qtde,seqComp,codColigada,codFilial)
...
//GERA O MOVIMENTO DE BAIXA DO ESTOQUE
function geraMovBaixa(numProcess, codColigada, codFilial, WKUser, codOrdem, idAtv, seqAtv, numOS){
	
	log.info("MOVIMENTO DE BAIXA DO ESTOQUE")
	
	log.info("Parâmetros: numProcess: "+numProcess+", codColigada: "+codColigada+", codFilial: "+codFilial+", WKUser: "+WKUser+", idAtv: "+idAtv+", seqAtv: "+seqAtv+
			", numOS: "+numOS+", codOrdem: "+codOrdem)
	...
	var lote = hAPI.getChildrenIndexes("LISTALOTESAUX_APONT");
	
	log.info("vou percorrer todos os itens da tabela de componentes")
	
	log.info("a tabela tem "+lote.length+" itens")
	...
	for (var i = 0; i < lote.length; i++){
		...
		var qtde = hAPI.getCardValue("QTDAPONTAAUXLOTE___" + lote[i])
		var idprd = hAPI.getCardValue("IDPRODUTOAUXLOTE___"+lote[i])
		var seqAtvComp = hAPI.getCardValue("SEQAUXLOTE___" + lote[i])
		var codLocComp = hAPI.getCardValue("CODLOCAUXLOTE___" + lote[i])
		var codigoPrd = hAPI.getCardValue("CODPRODUTOAUXLOTE___" + lote[i])
		var idLote = hAPI.getCardValue("IDLOTEAUXLOTE___" + lote[i])
		var custoMedio = hAPI.getCardValue("CUSTOMEDIOAUXLOTE___" + lote[i])

		log.info("estou no item "+lote[i]+" dos componentes e na atividade "+seqAtv)
		
		log.info("qtde: "+qtde+", idprd: "+idprd+", seqAtvComp: "+seqAtvComp+", codLocComp: "+codLocComp+", idLote: "+idLote+", custoMedio: "+custoMedio+", codigoPrd: "+codigoPrd)
		...
		if(seqAtv==seqAtvComp){
			...
			// SE QUANTIDADE DO COMPONENTE FOI INFORMADA
			if(!(qtde=="" || qtde==null || qtde==undefined || qtde=="null" || qtde==0 || qtde=="0")){
				...
				//var auxXML = componentesCodLoc(codColigada, codFilial, lote, seqAtv, idAtv, codLocComp, saldo, custoPosto, numOS)
				...
				try{
					...
					// SALVA O IDMOVBAIXA GERADO PARA TODOS OS COMPONENTES
					salvaIdMovBaixa(lote,seqAtv,codLocComp,result,idLote,idprd,qtde,seqAtvComp,codColigada,codFilial)
...
//GERA O MOVIMENTO DE BAIXA DO ESTOQUE
function geraMovBaixaPapc(numProcess, codColigada, codFilial, WKUser, numPlano){
	
	log.info("MOVIMENTO DE BAIXA DO ESTOQUE")
	
	log.info("Parâmetros: numProcess: "+numProcess+", codColigada: "+codColigada+", codFilial: "+codFilial+", WKUser: "+WKUser+", Plano: "+numPlano)
	...
	var c1 = DatasetFactory.createConstraint("TIPO","BAIXA","BAIXA",ConstraintType.MUST)
	var c2 = DatasetFactory.createConstraint("NUMPLANOCORTE",numPlano,numPlano,ConstraintType.MUST)
	...
	for (var i = 0; i < rows.length; i++){
		...
		var qtde = dataset.getValue(i,"QTDMPFINAL")
		var idprd = dataset.getValue(i,"IDPRD")
		var codLocComp = "23"
		var codigoPrd = dataset.getValue(i,"CODIGOPRD")
		var idLote = dataset.getValue(i,"IDLOTE")
		var custoMedio = dataset.getValue(i,"CUSTOMEDIO")
		var codOrdem = dataset.getValue(i,"CODORDEM")
		var idAtv = dataset.getValue(i,"IDATVORDEM")
		var numOS = dataset.getValue(i,"CODCCUSTO")

		log.info("estou no item "+i+" dos componentes e no plano "+numPlano)

		log.info("quantidade antes:"+qtde)

		qtde = Math.round(qtde*10000)/10000 

		qtde = qtde.toString()
		
		log.info("qtde: "+qtde+", idprd: "+idprd+", codLocComp: "+codLocComp+", idLote: "+idLote+", custoMedio: "+custoMedio+", codigoPrd: "+codigoPrd)
		
			
		log.info("achei componente do plano")
		...
		if(!(qtde=="" || qtde==null || qtde==undefined || qtde=="null" || qtde==0 || qtde=="0")){
			...
			try{
				...
				// SALVA O IDMOVBAIXA GERADO PARA TODOS OS COMPONENTES
				salvaIdMovBaixa(idLote,1,codLocComp,result,idLote,idprd,qtde,numPlano,codColigada,codFilial)
...
// 	var idMovBaixa = hAPI.getChildrenIndexes("IDMOVCOMPONENTES")

// 	var movimentos = new Array()
	
// 	log.info("vou percorrer todos os itens da tabela de movimentos de baixa")
	
// 	log.info("a tabela tem "+idMovBaixa.length+" itens")
...
// 		var idmov = hAPI.getCardValue("IMIDMOV___"+idMovBaixa[i]);
// 		var codColigada = hAPI.getCardValue("IMCODCOLIGADA___"+idMovBaixa[i]);
...
function deletaMovimentos(){
	...
	for (var i = 0; i < idMovBaixa.length; i++){
		...
		var idmov = idMovBaixa[i].IDMOV
		var codColigada = idMovBaixa[i].CODCOLIGADA
...
// SALVA O IDMOVBAIXA GERADO PARA TODOS OS COMPONENTES
function salvaIdMovBaixa(indexes,seqAtv,codLocCompAux,idMov,idLote,idprd,qtde,seqCompAtv,codColigada,codFilial){
	...
	log.info("idprd: "+idprd+", idLote: "+idLote+", qtde: "+qtde+", seqAtv: "+seqAtv+", seqCompAtv: "+seqCompAtv+", idMov: "+idMov+", coligada: "+codColigada+", filial: "+codFilial)
	...
	idMov = idMov.toString()
	codColigada = codColigada.toString()
	codFilial = codFilial.toString()
	...
	childData.put("IMCODCOLIGADA", codColigada)
    childData.put("IMCODFILIAL", codFilial)
    
    log.info("vou salvar na tabela")
    
    hAPI.addCardChild("IDMOVCOMPONENTES", childData)
    
    idMovBaixa.push({IDMOV:idMov,CODCOLIGADA:codColigada})
