...
function createDataset(fields, constraints, sortFields) {
    ...
    if(exec==""){
        ...
        myQuery = ";with cte as (SELECT * FROM Z_CRM_ML001005 WHERE IDCRIACAO="+idCriacao+" AND OS='"+numOS+"') "+
                  " SELECT *,CONCAT(INDICE,' - ',DESCRICAO) DESCITEM FROM Z_CRM_ML001005 WHERE NIVEL=(SELECT NIVEL FROM CTE) AND ( TIPODESENHO='NAOMANUFATURADO' OR TIPODESENHO='INDUSTRIALIZACAO') AND OS='"+numOS+"' "
        ...
        myQuery = ";with cte as (SELECT * FROM Z_CRM_EX001005 WHERE IDCRIACAO="+idCriacao+" AND OS='"+numOS+"' AND EXECUCAO="+exec+" AND ITEMEXCLUSIVO!=2) "+
                  " SELECT *,CONCAT(INDICE,' - ',DESCRICAO) DESCITEM FROM Z_CRM_EX001005 WHERE NIVEL=(SELECT NIVEL FROM CTE) AND ( TIPODESENHO='NAOMANUFATURADO' OR TIPODESENHO='INDUSTRIALIZACAO') "+
