...
function setSelectedZoomItem(selectedItem) {        
	...
	if(selectedItem.inputId.indexOf("CODMPCAD")!="-1"){
		...
		if(selectedItem['CODTB2FAT']!='0185'){
			...
			Swal.fire({
				icon: 'warning',
				title: 'Esse produto pertence ao grupo 0185',
				text: 'Produtos desse grupo não podem ser cadastrados em planos de corte'
			})

			$("#CODMATERIALCAD").val("")
			$("#IDMATERIALCAD").val("")
			window['CODMPCAD'].clear()
	...
	if(selectedItem.inputId.indexOf("LOTECAD")!="-1"){
		...
		if(selectedItem['CODTB2FAT']!='0185'){
			...
			//if(codMP=="" || codMP==null || codMP==undefined && !(numOS=="" || numOS==null || numOS==undefined)){
				...
				setZoomData("CODMPCAD",selectedItem['CODIGOPRD'] +" - "+ selectedItem['DESCRICAO'])
				$("#CODMATERIALCAD").val(selectedItem['CODIGOPRD'])
				$("#IDMATERIALCAD").val(selectedItem['IDPRD'])
				$("#CODSUCATA").prop("disabled",false)
				$("#RETALHOCAD").prop("disabled",false)
			...
			Swal.fire({
				icon: 'warning',
				title: 'Esse lote pertence a um produto do grupo 0185',
				text: 'Produtos desse grupo não podem ser cadastrados em planos de corte'
			})

			window["LOTECAD"].clear()
			$("#NUMLOTECAD").val("")
			$("#IDLOTECAD").val("")
			$("#SALDOLOTECAD").val("")
	...
	if(selectedItem.inputId.indexOf("PLANOCORTEED")!="-1"){
		...
		setTimeout(function(){
			...
			if(!(planoProgramado(selectedItem["NUMPLANOCORTE"]))){
				...
				if(RaDoPlanoGerada(selectedItem["NUMPLANOCORTE"])){
					// SE NÃO
					...
					Swal.fire({
						icon: 'warning',
						title: 'Esse Plano de Aproveitamento e Corte já foi gerada RA e não pode ser editado',
	...
	if(selectedItem.inputId.indexOf("LOTEED")!="-1"){
		...
		if(selectedItem['CODTB2FAT']!='0185'){
			...
			//if(codMP=="" || codMP==null || codMP==undefined && !(numOS=="" || numOS==null || numOS==undefined)){
				...
				setZoomData("CODMPED",selectedItem['CODIGOPRD'] +" - "+ selectedItem['DESCRICAO'])
				$("#CODMATERIALED").val(selectedItem['CODIGOPRD'])
				$("#IDMATERIALED").val(selectedItem['IDPRD'])
