...
function retornaArrayIndicesOrdenados(){
    ...
    var dataset = DatasetFactory.getDataset("dsEXEstruturaOrdenada",null,constraints,null)
    var row = dataset.values
...
function reindexacaoGeral(indice,indices,salvos){
	...
	if(!idTemMenorIrmaoEsq(indice,indices,salvos)){
        ...
        var indicesGlobal = retornaArrayIndicesOrdenados()
        
		// ALTERA O ÍNDICE ATUAL E DOS FILHOS PARA O NOVO GERADO
		alteraIndiceReind(novoIndice, indice, idCriacao, nivel, novaPosicao, 0, indices,indicesGlobal,'')
    ...
    if(idTemIrmaoDirGap(indice,indices,0)){
        ...
        var idCriacao = buscaIdCriacao(irmaoDir)
        ...
        // ALTERA O ÍNDICE ATUAL E DOS FILHOS PARA O NOVO GERADO
        alteraIndiceReind(novoIndice, irmaoDir, idCriacao, nivel, novaPosicao, 0, indices,indicesGlobal,'')
...
// ALTERA O ÍNDICE ATUAL E DOS FILHOS PARA O NOVO GERADO
function alteraIndiceReind(novoIndice,indiceAntigo,idCriacao,novoNivel,novaPosicao,id,indices,indicesGlobal,acao){
	...
	while(idTemFilho(indiceAntigo,indicesGlobal,0)){
		...
		alteraIndiceReind(novoIndiceFilho,filho,idCriacaoFilho,novoNivelFilho,posicaoFilho,id+1,indices,indicesGlobal,acao)
	...
	if(!(id==0)){
		...
		while(idTemIrmaoDir(indiceAntigo,indicesGlobal,0)){
			...
			// CHAMA FUNÇÃO PARA ALTERAR OS ÍNDICES
			alteraIndiceReind(novoIndiceIrmao,irmao,idCriacaoIrmao,novoNivel,posicaoIrmao,id+1,indices,indicesGlobal,acao)
    ...
    id = indicesGlobal.indexOf(indiceAntigo)
    indicesGlobal[id]=novoIndice
    ...
    //indicesGlobal = retornaArrayIndicesOrdenados()
...
function trocaIndice(indice,idCriacao){
	...
	if(nivel=="" || nivel==undefined || nivel==null){
		...
		reloadZoomFilterValues("F_INDICE_REF","OS,"+numOS+"NIVEL,"+nivel+",EXECUCAO,"+execucao)
		
	} else{
		
		// FAZ O RELOAD ZOOM NA CONSULTA DOS INDICES
		reloadZoomFilterValues("F_INDICE_REF","OS,"+numOS+",EXECUCAO,"+execucao)
